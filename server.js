...
app.get('/api/tasks/filter', auth.verifyToken, async (req, res) => {
    try {
        const { completed } = req.query;
        const { id } = req.user;
        const { data, error } = await supabase.from('task_manager_tasks').select('*').eq('user_id', id).eq('completed', completed);
        if (error) {
            return res.status(400).json({ error: error.message });
        }
        res.json(data);
